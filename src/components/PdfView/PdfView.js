...
import { ListGroup, Container, Col, Row, Form, Button, OverlayTrigger, Tooltip } from "react-bootstrap";
            ...
            <Col className="col-lg-9 pdf-viewer-container">
              {fileUrl ? (
                <>
                  <Viewer
                    fileUrl={fileUrl}
                    plugins={[highlightPluginInstance, pageNavigationPluginInstance, jumpToPagePluginInstance]}
                  />
                </>
              ) : (
                <>
              ...
              <ListGroup as="ol" numbered>
                {
                  areas.previews.map(
                    (preview, ind) =>
                      <ListGroup.Item onClick={() => jumpResult(ind)} style={{boxShadow: (ind === currentIndex) ? "0 4px 8px rgba(0, 0, 0, 0.4)" : "none"}} as="li" key={ind} className="right-sidebar-button mx-2 my-1">{preview}</ListGroup.Item>
                  )
                }
              </ListGroup>
            </Col>
              ...
              <Container fluid className="searchbar-container-inner">
                <div className="d-flex mb-2 align-items-center " style={{width: "100%"}}>
                  <div style={{marginRight: "0.5rem"}}>
                    <SearchBarButton text="Delete File" IconComponent={Icon.Trash} onClickFunc={() => {}}/>
                  </div>
                  <div style={{marginRight: "auto", display: "flex", alignItems: "center"}}>
                    <input onKeyDown={handleJumpPage} defaultValue={1} style={{marginRight: "0.2rem"}} id="page-number-input" className="plain-input" />
                  ...
                  <SearchBarButton text="Next Result" IconComponent={Icon.ArrowRight} onClickFunc={() => moveResult(true)}/>
                </div>
        ...
        <CurrentPageLabel>
          {
            (props) => {
              ...
              document.getElementById("page-number-input").value = props.currentPage + 1
              document.getElementById("total-pages-span").innerText = `of ${totalPages}`
            }
          }
        </CurrentPageLabel>
